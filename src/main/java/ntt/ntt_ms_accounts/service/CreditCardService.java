...
public class CreditCardService {
    ...
    public Mono<CreditCard> charge(String id, BigDecimal amount, String ref) {
    ...
    public Mono<CreditCard> payment(String id, BigDecimal amount) {
        return cardRepo.findById(id)
                .switchIfEmpty(Mono.error(new IllegalArgumentException("card not found")))
                .flatMap(card -> {
                    BigDecimal newAvailable = card.getAvailable().add(amount);
                    if (newAvailable.compareTo(card.getCreditLimit()) > 0) newAvailable = card.getCreditLimit();
